...
- Name each new task ID `YYYYMMDD-HHmmss-short-semantic-slug`, using the actual creation time in the current environment's local timezone and a short, lowercase, hyphen-separated description of the objective, for example `20260906-161115-loop-completion`. In the document, keep `Task ID`, `Created`, and `Updated` as plain field names; format both timestamps as local ISO 8601 values with their UTC offset. Set both timestamps at creation, keep `Created` immutable, and refresh `Updated` whenever the task record meaningfully changes. Allocate the directory without overwriting an existing one; if the same timestamp and slug collide, append `-02`, `-03`, etc. Keep the ID stable when the title changes or work resumes. Rename an existing ID only when the user requests a naming migration, updating references and recording the previous ID.
