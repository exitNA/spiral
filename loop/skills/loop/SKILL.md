...
2. Maintain a dynamic work list linked to the stable requirement IDs, following [references/memory-compression.md](references/memory-compression.md). Publish and refresh this list through Codex’s native plan/checklist tool, including the reference’s item, dependency, blocker, and next-action mapping. Resolve the host-specific tool and schema through [references/codex-runtime.md](references/codex-runtime.md). Markdown checkboxes and document panels are not substitutes; if the tool is unavailable, record and report native synchronization as blocked while continuing other authorized work. Turn discovered gaps into implementation or verification work; keep reasons and coverage when splitting, replacing, cancelling, or reopening items. A coarse whole is a milestone; preserve the list across delegation and context boundaries and continue until the requested whole is verified.
